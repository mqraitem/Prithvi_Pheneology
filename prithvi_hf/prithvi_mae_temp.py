...
class PatchEmbed(nn.Module):
    """3D version with optional space-to-depth patchification (no spatial mixing)."""
    def __init__(
        self,
        ...
        flatten: bool = True,
        bias: bool = True,
        mode: str = "space_to_depth",   # <— new: "space_to_depth" or "conv"
    ):
        super().__init__()
        ...
        elif mode == "space_to_depth":
            pt, ph, pw = patch_size
            # 1x1x1 projection after pure rearrange (no spatial mixing)
            self.proj = nn.Conv3d(in_chans * pt * ph * pw, embed_dim,
    ...
    def forward(self, x):
        ...
        if T % pt or H % ph or W % pw:
            logging.warning(f"Input {x.shape[-3:]} not divisible by patch size {self.patch_size}.")

        if self.mode == "conv":
            x = self.proj(x)
        else:
            # space-to-depth: (B, C, T, H, W) -> (B, C*pt*ph*pw, T/pt, H/ph, W/pw)
            x = rearrange(x, "b c (t pt) (h ph) (w pw) -> b (c pt ph pw) t h w",
                          pt=pt, ph=ph, pw=pw)
            x = self.proj(x)
