...
class UNet3D(nn.Module):
    def __init__(self, in_channel, n_classes, timesteps, dropout):
        ...
        # self.logsoftmax = nn.LogSoftmax(dim=1)   # REMOVED
        self.dropout = nn.Dropout(p=dropout, inplace=True)
        
    def forward(self, x):
        # x = x.cuda()  # REMOVED: keep device-agnostic
        en3 = self.en3(x)
        ...
        # final = self.logsoftmax(final)  # REMOVED
        
        return final   # (B, n_classes, H, W) raw regression
