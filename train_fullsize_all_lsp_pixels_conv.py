...
from utils import segmentation_loss_pixels, eval_data_loader, get_masks_paper, save_checkpoint,str2bool
...
def main():
	...
	device = "cuda"
	model = GlobalTemporalConvNetFlexible(
		in_ch=6,
		num_classes=4,
		t_steps=12,     # must match input T
		hidden=128,
		n_layers=3,
		dropout=0.1
	)
	...
	for epoch in range(config["training"]["n_iteration"]):
		...
		for j,batch_data in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
			...
			optimizer.zero_grad()
			out=model(input, processing_image=False)
